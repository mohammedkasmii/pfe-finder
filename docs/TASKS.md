...
| M2 | Database and ingestion | Claude | M1 | REVIEW | Reproducible Supabase migrations/RLS, adapter contract, three configured sources, classification fixtures, safe URL/text normalization, idempotent upsert, complete-scan semantics, and collector tests pass. |
...
| R2 | Review M2 | Codex | M2 in REVIEW | CHANGES_REQUESTED | Sources, schema, RLS, normalization, idempotency, and failure semantics reviewed. |
