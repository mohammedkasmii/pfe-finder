...
| --- | --- | --- | --- | --- | --- |
| M1 | Project foundation and design system | Claude | Documentation | REVIEW | Next.js/TypeScript/Tailwind scaffold, bilingual shell, responsive accessible visual system, environment validation, test/lint/typecheck/build scripts, `.env.example`, and CI workflow pass locally where supported. |
| M2 | Database and ingestion | Claude | M1 | BLOCKED | Reproducible Supabase migrations/RLS, adapter contract, three configured sources, classification fixtures, safe URL/text normalization, idempotent upsert, complete-scan semantics, and collector tests pass. |
| M3 | Search and offer experience | Claude | M2 | BLOCKED | Validated cursor API, filters in URL, offer list/detail pages, local favorites, bilingual states, freshness display, rate limiting, and component/API tests pass. |
...
| R1 | Review M1 | Codex | M1 in REVIEW | CHANGES_REQUESTED | Diff, architecture alignment, UX/accessibility baseline, and checks reviewed; result recorded here and in handoff. |
| R2 | Review M2 | Codex | M2 in REVIEW | BLOCKED | Sources, schema, RLS, normalization, idempotency, and failure semantics reviewed. |
| R3 | Review M3 | Codex | M3 in REVIEW | BLOCKED | Product behavior, API validation, performance, accessibility, and browser flows reviewed. |
